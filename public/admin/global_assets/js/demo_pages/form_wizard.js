...
var FormWizard = function() {
    ...
    var _componentWizard = function() {
        ...
        $('.steps-validation').steps({
            ...
            titleTemplate: '<span class="number">#index#</span> #title#',
            labels: {
                previous: '<i class="icon-arrow-left13 mr-2" /> Sebelum',
                next: 'Selanjutnya <i class="icon-arrow-right14 ml-2" />',
                finish: 'Simpan Transaksi <i class="icon-arrow-right14 ml-2" />'
            },
            transitionEffect: 'fade',
            autoFocus: true,
            ...
            onFinished: function (event, currentIndex) {
              // alert($(".nm_supplier").val());
              var me = $(this),
              url = me.attr('action');
              var id_supplier = $(".nm_supplier").val();
              var keterangan = $(".keterangan").val();
              var tgl_pengadaan = $(".tgl_pengadaan").val();
              if (status=='isi') {
                 $.post( url, {id_supplier:id_supplier,tgl_pengadaan:tgl_pengadaan,keterangan:keterangan})
                 .done(function( data ) {
                    Swal.fire(
                        'Good Job !',
                        ...
                        'success'
                          ...
                          if (result.value) {
                            window.location.replace(me.attr('lempar'));

                        }
                    });
                        $.get($(".nm_supplier").attr("link")+"/"+$(".nm_supplier").val(), function( data ) {
                            $( ".list_pengadaan" ).html( data );
                        });
                    });
             }else{

                Swal.fire(
                    'List data belum terisi !',
                    'Mohon tekan tombol hijau !',
                    ''
                    ).then((result) => {
                      if (result.value) {
